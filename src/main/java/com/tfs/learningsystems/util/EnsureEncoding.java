 ...
 * @see https://gist.github.com/maerzbow/1896f9fc8fbc8f06804a
 */

@Slf4j
public class EnsureEncoding {

  public static final Charset[] TRY_ENC_UTF8_ISO88591_UTF16LE_UTF16BE = new Charset[]{UTF_8,
      ISO_8859_1, UTF_16LE, UTF_16BE};
  public static final Charset[] TRY_ENC_UTF16LE_UTF16BE_UTF8_ISO88591 = new Charset[]{UTF_16LE,
      UTF_16BE, UTF_8,
      ISO_8859_1};
  ...
  /**
   * @param encodingsToTry a set of charsets to be passed if the default ENCODINGS_TO_TRY does
  ...
  /**
   * will try to convert the given chars to a valid string. no additional content check will be
   * done
   *
   * @param chars characters in an unknown charset
   * @return the chars converted to a String decoded by the first matching {@link Charset}
   * @throws Exception if converting the chars to a String was not possible
   */
  ...
  /**
   * will try to convert the given chars to a valid string. will check for every valid conversion if
   * the converted String contains 'hasToContain' as additional check, if not it will continue with
   * the next encoding
   *
   * @param chars characters in an unknown charset
   * @param hasToContain checks if the converted String contains this value
   * @return the chars converted to a String decoded by the first matching {@link Charset}
   * @throws Exception if converting the chars to a String was not possible
   */
  ...
  /**
   * will try to convert the given chars to a valid string. will check for every valid conversion if
   * the given {@link ContentCheck} is valid, if not it will continue with the next encoding
   *
   * @param chars characters in an unknown charset
   * @param check this instance will be called for every valid conversion
   * @return the chars converted to a String decoded by the first matching {@link Charset}
   * @throws Exception if converting the chars to a String was not possible
   */
  ...
  public interface ContentCheck {
    ...
    /**
     * @param content additionally checks this String for validity
